...
The following issues were found in the implemented files and fixed directly:

1. **`backend/app/main.py` — file was truncated; completed it.** The file contained only the imports; the `lifespan` function, `create_app()` factory, CORS middleware, and the module-level `app = create_app()` were all missing. Uvicorn boots from `app.main:app` (see `infra/docker/backend.Dockerfile`), so the container would have crashed immediately with an `AttributeError` — there was no `app` object to import. Restored the full lifespan (client creation on startup, reverse-order close on shutdown) and app factory per this guide.

2. **`backend/app/deps.py` — `get_db` returned the Neo4j driver instead of a DB session, and `get_neo4j` was missing.** As written, every route asking for `Depends(get_db)` would have received an `AsyncDriver` instead of an `AsyncSession`, and every route importing `get_neo4j` (papers, graph, latex, agent) would have failed at import time with an `ImportError`. Rewrote `get_db` as the generator dependency from this guide (opens an `AsyncSession` from `app.state.session_factory`, yields it, closes it after the response) and added the separate `get_neo4j` accessor.

3. **`backend/app/db/postgres.py` — two bugs in `check_embedding_dimension`.**
   - The table name was misspelled `paper_chuncks` in three places (the `to_regclass` probe, the `pg_attribute` lookup, and the error message). The real table is `paper_chunks`, so the probe would always return `NULL` and the dimension check would silently skip forever — exactly the "storing garbage vectors" failure this guard exists to prevent.
   - The two-part SQL string was concatenated without a space, producing `...FROM pg_attributeWHERE...`, which is invalid SQL. Added the missing trailing space.

4. **`backend/alembic.ini`, `backend/alembic/env.py`, `backend/alembic/script.py.mako`, `backend/alembic/versions/0001_initial_schema.py` — created; they did not exist.** These files are part of this guide but were never written, so `alembic upgrade head` had nothing to run and the database had no schema (which also means the app's startup dimension check and every query would fail). Created all four verbatim from this guide.

5. **`backend/app/app.py` — deleted.** This was the placeholder FastAPI app with a stub `/api/health` route from the bootstrap guide. Nothing references it (Docker and compose run `app.main:app`), and keeping a second `FastAPI()` instance next to the real one invites confusion about which app is being served. The real health route lives in `app/api/routes/health.py`.

No changes were needed to `config.py`, `logging.py`, or `db/models.py` — they match this guide.

### Addendum (first real boot, 2026-07-05)

5. **`backend/app/db/postgres.py` — the guide's own typmod formula is wrong; removed the `- 4` offset.** This guide (and the original implementation) computed `actual_dim = typmod - 4`, claiming pgvector stores the dimension "with a 4-byte header offset." That offset (`VARHDRSZ`) applies to `varchar` typmods, **not** pgvector: pgvector's `typmod_in` stores the dimension directly, so `vector(1536)` has `atttypmod = 1536` (verified against the live database: `SELECT atttypmod, format_type(atttypid, atttypmod) ...` returns `1536 | vector(1536)`). With the `- 4`, the startup check computed 1532, mismatched `EMBEDDING_DIM=1536`, and crashed the app on its first boot against a migrated database — the guard's one false positive killed the process it was protecting. Fixed to `actual_dim = typmod`.
