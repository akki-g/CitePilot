...
1. **`backend/app/api/routes/projects.py` — three issues in project creation/listing:**
   - The second bootstrap file was created as `ProjectFile(project_if=id, ...)` — a typo'd keyword (`project_if`) whose value was the `id` *builtin function*. SQLAlchemy rejects unknown kwargs, so **every** `POST /api/projects` raised `TypeError` and no project could ever be created. Fixed to `project_id=project.id`.
   - `list_projects` omitted `description` from the response dict; the guide (and the frontend's project list) includes it. Added.
   - `create_project` was missing `status_code=201`; creation endpoints should return 201, and clients checking for it would treat the 200 as unexpected. Added.

No changes were needed to `router.py`, `routes/health.py`, `routes/files.py`, `routes/papers.py`, `routes/jobs.py`, `routes/graph.py`, `routes/agent.py`, `routes/latex.py`, `workers/arq_app.py`, or `workers/jobs.py` — they match this guide.

Related fixes made in other modules that this module depends on (documented in their own guides): `app/main.py` was completed and `app/deps.py` gained the missing `get_neo4j`/session-yielding `get_db` (guide 01) — without those, none of these routes could even import; `latex/compiler.py` now writes status `"completed"` (guide 05), which is what the compilation polling route effectively reports to the UI.
