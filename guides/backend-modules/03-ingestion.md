...
1. **`backend/app/ingestion/openalex.py` — renamed `_cached_key` to `_cache_key`.** `_cached_get` calls `self._cache_key(path, params)`, but the static method was typed as `_cached_key`. Every OpenAlex request (search or work fetch) would have raised `AttributeError` before touching the network. Also fixed the `OPENALX_MAILTO` typo in the constructor's error message so the message names the real env var.

2. **`backend/app/ingestion/upsert.py` — the DOI branch of `find_existing_paper` was missing `.scalar_one_or_none()`.** The code did `paper = (await session.execute(select(Paper).where(Paper.doi == np.doi)))` and then `if paper: return paper`. `session.execute()` returns a `Result` object, which is always truthy — even when zero rows match. So for *any* import with a DOI, the function returned the `Result` object instead of a `Paper` or `None`, and `upsert_paper` would then crash in `_enrich` trying to read `paper.openalex_id` off a `Result`. This silently broke the entire dedup gate for DOI-bearing papers (the primary match path). Added the missing `.scalar_one_or_none()` to match the other three match branches.

No changes were needed to `crossref.py`, `semantic_scholar.py`, `normalize.py`, or `bibtex.py` — they match this guide.
