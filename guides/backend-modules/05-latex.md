...
1. **`backend/app/latex/sanitizer.py` — `SAFE_PATH_RE` was defined but never applied.** The guide's "reject unsupported characters" check was missing, so paths containing spaces, `$`, `~`, backticks, and other characters outside the safe set sailed through the sanitizer. This is the module whose entire job is loud rejection of suspicious paths, and this check is the character-level half of it (the segment-level half — `..`, hidden files — was present). Added `if not SAFE_PATH_RE.match(path): raise UnsafePathError(...)` before path parsing.

2. **`backend/app/latex/patcher.py` — four bugs:**
   - `InsertAfterPatch` was missing the `base_version: int` field. `_check_version` reads `patch.base_version`, so every `insert_after` patch raised `AttributeError` — and worse, if validation had been skipped, insert patches would have bypassed the stale-version concurrency safety net entirely. Both the agent tool registry and the MCP tool docs advertise `base_version` on `insert_after`. Added the field.
   - `apply_patch` incremented `file_version += 1` — an undefined local name, so applying any patch raised `NameError` *after* mutating `file.content` in the session. Fixed to `file.version += 1`.
   - `PatchError.__init__` stored `self.details = details` instead of `details or {}`, so omitting details left `None` where consumers expect a dict. Fixed.
   - The file-not-found error code was `"file not found"`; error codes are machine-readable identifiers and the other codes (`anchor_not_found`, `stale_version`, …) are snake_case. Changed to `"file_not_found"`.

   Note: the implementation also defines `PATCH_ADAPTER = TypeAdapter(Patch)`, which is not in this guide's snippet but *is* imported by `agent/tools.py` and `agent/orchestrator.py` (guide 06). Kept — it is the right home for it.

3. **`backend/app/latex/compiler.py` — success status was `"compiled"` instead of `"completed"`.** The jobs table, tool-call records, and the compilation status route all use `"completed"` as the terminal success value, and the frontend will poll for it. A successful compile would have looked permanently unfinished to the UI. Fixed to `"completed"`.

No changes were needed to `infra/docker/worker.Dockerfile` — it matches this guide.
