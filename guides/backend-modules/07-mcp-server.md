...
1. **`backend/pyproject.toml` — added the missing `mcp>=1.2` dependency.** This guide says to add the official MCP Python SDK to the backend dependencies, but it was never added. `app/mcp_server/server.py` and `tools.py` both import `mcp.server.fastmcp`, so launching the MCP server (`python -m app.mcp_server.server`) would fail with `ModuleNotFoundError: No module named 'mcp'` until the image is rebuilt with the new dependency. Marked with a `# fix:` comment in the file.

No changes were needed to `backend/app/mcp_server/server.py` or `backend/app/mcp_server/tools.py` — they match this guide exactly.
