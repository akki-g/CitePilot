...
1. **`backend/app/graph/queries.py` — added the missing `bibliographic_coupling` function.** The `_BIBLIOGRAPHIC_COUPLING` Cypher string was typed in, but the function that executes it was skipped. Both `retrieval/graph_search.py` and `agent/tools.py` call `queries.bibliographic_coupling(...)`, so hybrid retrieval and the agent's `related_work_search` tool would have crashed with `AttributeError` on first use. Added the function from this guide (empty-seed short-circuit, `signal="coupling"`, `shared_reference_count` feature).

2. **`backend/app/graph/sync.py` — four bugs in `sync_paper`:**
   - The authors query filtered `.where(PaperAuthor.paper_id == paper)`, comparing the column to the ORM `Paper` *object* instead of the `paper_id` UUID. SQLAlchemy would fail to coerce a `Paper` instance into a UUID bind parameter, so syncing any paper with authors would error. Fixed to `== paper_id`.
   - `driver.sessoin()` was a typo for `driver.session()` — an instant `AttributeError` on every `sync_paper` call.
   - The paper MERGE Cypher read `MERGE (p:Paper {id" $id})`; the `"` should be `:`. Neo4j would reject this with a syntax error.
   - The Cypher `SET` clause references `$cited_by_count`, but the `cited_by_count=paper.cited_by_count` keyword argument was missing from the `graph.run(...)` call. The Neo4j driver raises "Expected parameter(s): cited_by_count" when a query references an unbound parameter. Added the argument.

   Net effect before the fix: every single-paper sync (and therefore every import and every resync) failed at the first `graph.run`.

3. **`backend/app/graph/resync.py` — `async with session_factory as session:` was missing the call parentheses.** `async_sessionmaker` must be *called* to produce a session; the factory object itself is not an async context manager, so `make resync-graph` would crash with `TypeError`. Fixed to `session_factory()`.

No changes were needed to `neo4j_client.py`, `schema.py`, or `infra/scripts/init_neo4j.cypher` — they match this guide.
